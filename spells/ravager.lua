...
local menu_elements_ravager_base = 
{
    ...
    min_max_targets       = slider_int:new(0, 30, 5, get_hash(my_utility.plugin_label .. "min_max_number_of_targets_for_ravager_base"))
}

local function menu()
    
    if menu_elements_ravager_base.tree_tab:push("Ravager") then
        menu_elements_ravager_base.main_boolean:render("Enable Spell", "")

        if menu_elements_ravager_base.main_boolean:get() then
            local dropbox_options = {"No filter", "Elite & Boss Only", "Boss Only"}
            menu_elements_ravager_base.filter_mode:render("Filter Modes", dropbox_options, "")
        end

        if menu_elements_ravager_base.main_boolean:get() then
            menu_elements_ravager_base.min_max_targets:render("Min Enemies Around", "Amount of targets to cast the spell")
        end

        menu_elements_ravager_base.tree_tab:pop()
    end
end

local next_time_allowed_cast = 0.0;
local spell_id_ravager = 1862773;


local function logics()

    local menu_boolean = menu_elements_ravager_base.main_boolean:get();
    ...
    if  ((filter_mode == 1 and elite_units >= 1 and not elite_collision)
        ...
        or (units >= menu_elements_ravager_base.min_max_targets:get())
        then
            if cast_spell.self(spell_id_ravager, 0.000) then
            -- ignore global cooldown -- test 04/06/2024 -- qqt
            local current_time = get_time_since_inject();
            next_time_allowed_cast = current_time + 0.4;
            console.print("Casted Ravager")
            return true;
        end
    end

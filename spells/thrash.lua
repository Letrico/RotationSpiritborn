...
local menu_elements_thrash_base =
{
    ...
    main_boolean        = checkbox:new(true, get_hash(my_utility.plugin_label .. "thrash_main_bool_base")),
    use_as_filler_only  = checkbox:new(true, get_hash(my_utility.plugin_label .. "thrash_use_as_filler_only")),
}
...
local function menu()
    if menu_elements_thrash_base.tree_tab:push("Thrash")then
        ...
        if menu_elements_thrash_base.main_boolean:get() then
            menu_elements_thrash_base.use_as_filler_only:render("Filler Only", "Prevent casting with a lot of spirit")
        end
...
local function logics(target)
    ...
    local is_filler_enabled = menu_elements_thrash_base.use_as_filler_only:get();  

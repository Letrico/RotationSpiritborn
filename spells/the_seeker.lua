...
local menu_elements = {
    the_seeker_submenu           = tree_node:new(1),
    the_seeker_boolean           = checkbox:new(true, get_hash(my_utility.plugin_label .. "the_seeker_boolean_base")),
    the_seeker_mode              = combo_box:new(0, get_hash(my_utility.plugin_label .. "the_seeker_cast_modes_base")),
    min_hits_slider             = slider_int:new(0, 30, 5, get_hash(my_utility.plugin_label .. "the_seeker_min_hits_slider_base")),

    allow_elite_single_target   = checkbox:new(true, get_hash(my_utility.plugin_label .. "allow_elite_single_target_base")),
    effect_size_affix_mult      = slider_float:new(0.0, 200.0, 0.0, get_hash(my_utility.plugin_label .. "the_seeker_effect_size_affix_mult_slider_base")),
}

local function menu()
    if menu_elements.the_seeker_submenu:push("The Seeker") then
        menu_elements.the_seeker_boolean:render("Enable The Seeker Cast", "")

        if menu_elements.the_seeker_boolean:get() then
            -- create the combo box elements as a table
            local dropbox_options = {"Combo & Clear", "Combo Only", "Clear Only"}
            menu_elements.the_seeker_mode:render("Cast Modes", dropbox_options, "")
        ...
        menu_elements.the_seeker_submenu:pop()
    end
end
...
local function logics()
    ...
    local menu_boolean = menu_elements.the_seeker_boolean:get();
    ...
    local spell_effect_radius_raw = 3.90 -- radius of the_seeker
    ...
    if cast_spell.position(the_seeker_spell_id, best_cast_position, 0.40) then
        
        local current_time = get_time_since_inject();
        last_the_seeker_cast_time = current_time + value;
        console.print("Casted The Seeker, Target " .. best_target:get_skin_name() .. " Hits: " .. best_cast_hits);

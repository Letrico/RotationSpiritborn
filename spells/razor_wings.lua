...
local menu_elements =
{
    ...
    main_boolean        = checkbox:new(true, get_hash(my_utility.plugin_label .. "razor_wings_base_main_bool")),

    trap_mode         = combo_box:new(0, get_hash(my_utility.plugin_label .. "razor_wings_base_mode")),
    keybind               = keybind:new(0x01, false, get_hash(my_utility.plugin_label .. "razor_wings_base_keybind_pos")),
    keybind_ignore_hits   = checkbox:new(true, get_hash(my_utility.plugin_label .. "razor_wings_base_keybind_ignore_min_hitstrap_base_pos")),

    min_hits              = slider_int:new(1, 20, 4, get_hash(my_utility.plugin_label .. "razor_wings_base_min_hits_to_casttrap_base_pos")),
    
    allow_percentage_hits = checkbox:new(true, get_hash(my_utility.plugin_label .. "allow_percentage_hits_razor_wings_base_pos")),
    min_percentage_hits   = slider_float:new(0.1, 1.0, 0.50, get_hash(my_utility.plugin_label .. "min_percentage_hits_razor_wings_base_pos")),
    soft_score            = slider_float:new(2.0, 15.0, 6.0, get_hash(my_utility.plugin_label .. "min_percentage_hits_razor_wings_base_soft_core_pos")),

    spell_range   = slider_float:new(1.0, 15.0, 10.0, get_hash(my_utility.plugin_label .. "razor_wings_base_spell_range")),
    spell_radius   = slider_float:new(0.50, 5.0, 1.50, get_hash(my_utility.plugin_label .. "razor_wings_base_spell_radius")),
}
...
local spell_id_razor_wings = 1871807;
...
local function logics(entity_list, target_selector_data, best_target)
    ...
    if cast_spell.position(spell_id_razor_wings, cast_position, 0.4)then
        local current_time = get_time_since_inject();
        next_time_allowed_cast = current_time + 0.4;
            
        console.print("Casted Razor Wings");
        return true;
    end
    return false;
end
